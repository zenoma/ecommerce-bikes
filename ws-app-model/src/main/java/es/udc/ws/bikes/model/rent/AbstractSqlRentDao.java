...
public abstract class AbstractSqlRentDao implements SqlRentDao {
	...
	public void update(Connection connection, Rent rent) throws InstanceNotFoundException {
		...
		String queryString = "UPDATE Rent" + "SET userEmail = ?, modelName = ?, creditCard = ?, "
				+ "startRentDate = ?, finishRentDate = ?, numberOfBikes = ?, rentDate = ? FROM Rent "
				+ "WHERE rentId";
		...
		try (PreparedStatement preparedStatement = connection.prepareStatement(queryString)){
			...
			Timestamp timestamp = rent.getStartRentDate() != null ? new Timestamp(rent.getStartRentDate().getTime().getTime())
					: null;
			...
			timestamp = rent.getRentDate() != null ? new Timestamp(rent.getRentDate().getTime().getTime())
					: null;
			preparedStatement.setTimestamp(i++, timestamp);
